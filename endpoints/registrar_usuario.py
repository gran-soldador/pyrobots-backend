...
@router.post("/user/registro_de_usuario/", tags=["User Methods"], name="Registro de Usuarios")
async def registro_usuario(name: str, password: str, email: str):
    with db_session:
        if len(name) > MAX_NICKNAME_SIZE:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Password inválido, el password requiere al menos una mayuscula, una minusucula y un numero.")
        elif not ("@" in email):
